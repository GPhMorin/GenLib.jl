"""
ancestor(genealogy::Dict{Int64, Individual}, ID::Int64)
...
"""
ancestor(genealogy::Dict{Int64, Individual}, IDs::Set{Int64})

Takes a `genealogy` dictionary and a set of `IDs` and returns a vector of all of their ancestors.
"""
function ancestor(genealogy::Dict{Int64, Individual}, IDs::Vector{Int64})
    ancestors = ∪([ancestor(genealogy, ID) for ID in IDs]...)
    sort!(ancestors)
end

"""
findMRCA(genealogy::Dict{Int64, Individual}, IDs::Vector{Int64})

Takes a `genealogy` dictionary and a vector of `IDs` and returns a vector of most recent common ancestors.
"""
function findMRCA(genealogy::Dict{Int64, Individual}, IDs::Vector{Int64})
    ancestors = [ancestor(genealogy, ID) for ID in IDs]
